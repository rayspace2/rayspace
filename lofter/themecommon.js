...
//	},3000);
	...
	//LOF-605:PC端门板功能 (前端修改)
	function initUserSplashLayer(_imgwidth,_imgHeight){
		var _ntmp = getElementsByClassName(document.body,'m-usersplash');
		var _usersplashNode;
		if(!!_ntmp && !!_ntmp[0]){
			_usersplashNode = _ntmp[0];
			...
			if(!!_img && _img.parentNode.parentNode ==_usersplashNode && !!_img.src){
				...
				var _bodyOverflowY = _body0.style.overflowY;
				...
				addEvent(_usersplashNode, 'click', function(_event){
					stopEvent(_event);
					_usersplashNode.style.display = 'none';
					_body0.style.paddingRight = 0;
					_body0.style.overflowY = _bodyOverflowY;
				});
			}
		}
